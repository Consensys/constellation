...
errorOut s = ioError (userError $ s ++ ". " ++ usageInfo header options) >> error ""
  where
    header = "Usage: constellation-node [OPTION...] [config file containing options] (If a configuration file is specified, any command line options will take precedence.)"
