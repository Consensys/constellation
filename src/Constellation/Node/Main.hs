...
defaultMain = do
    ...
    if cfgJustShowVersion cfg
        then putStrLn $ ("Constellation Node " ++ T.unpack version)
        else do
...
setupCrypt pubs privs mpwds = do
    pwds <- case mpwds of
        Just passPath -> (map (Just . T.unpack) . T.lines) <$>
            TIO.readFile passPath
...
setupStorage defaultpath = case break (== ':') defaultpath of
    ("bdb",     ':':path) -> sqliteStorage path
    ("dir",     ':':path) -> sqliteStorage path
    ("leveldb", ':':path) -> sqliteStorage path
    ("memory",  _       ) -> sqliteStorage defaultpath
    ("sqlite",  ':':path) -> sqliteStorage path
    _                     -> sqliteStorage defaultpath  -- Default
